...
- **Combined deadline/overdue overview** at the very top — one global overview containing overdue TODO items and, when enabled, upcoming deadlines. Overdue rows are sorted most overdue first and show days overdue + kind badge (DEADLINE/SCHEDULED) + clickable TODO badge before the title. Upcoming deadline rows suppress only the matching deadline appearance from the calendar so the same deadline is not presented twice; any separate `SCHEDULED` appearance for the same task remains visible. Due text is rendered as `Today` or compact day counts like `12d`, with urgency colors that progress from red to orange to yellow to a calmer tone as the due date gets farther away.
