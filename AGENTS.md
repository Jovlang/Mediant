...
- **Combined deadline/overdue overview** at the very top — one global overview containing overdue TODO items and, when enabled, upcoming deadlines due within the next 30 days. Overdue rows are sorted most overdue first; each row shows a clickable `-Nd` timestamp (which also acts as the TODO toggle) followed by the title and tags. The timestamp is colored by type: deadline overdue rows use critical red (`--deadline-urgency-critical`), scheduled overdue rows use a distinct slate-blue tone (`--overdue-scheduled`). Upcoming deadline rows suppress only the matching deadline appearance from the calendar so the same deadline is not presented twice; any separate `SCHEDULED` appearance for the same task remains visible. Due text is rendered as `Today` or compact day counts like `12d`, with urgency colors that progress from red to orange to yellow to a calmer tone as the due date gets farther away.
...
- **Deadline placement toggle** — settings toggle switches upcoming deadlines between the combined overview (`Show deadlines at top` / `Vis frister øverst`) and the day cards (`Show deadlines in calendar` / `Vis frister i kalenderen`). When upcoming rows are shown in the overview, their matching in-calendar deadline appearances are suppressed; when they are shown in the calendar, the overview rows are hidden. Overdue deadline appearances remain visible in both the overview and calendar when present. Preference persists in localStorage (`mediant-hide-deadlines`).
