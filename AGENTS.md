...
- **Shorthand date input** — add/edit date fields accept `DD`, `DD/MM`, `DD/MM/YY`, `DD/MM/YYYY`, `DD mon`, `DD mon YY`, `+N`, and weekday abbreviations in all four supported locales: `mon`..`sun` (EN), `man`..`søn` (NB), `lun`..`dom` (IT), `mo`..`so` (DE). All locale abbreviations are recognized regardless of the active locale. Ambiguous forms without a year resolve to the next future occurrence, and 2-digit years are interpreted in the current century.
