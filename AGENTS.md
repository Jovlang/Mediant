...
- **Hide upcoming deadlines** — settings toggle hides the optional upcoming-deadline rows in the combined overview. When upcoming rows are shown in the overview, their matching in-calendar deadline appearances are suppressed; when they are hidden, those deadline appearances remain in the day cards. Overdue deadline appearances remain visible in both the overview and calendar when present. Preference persists in localStorage (`mediant-hide-deadlines`).
